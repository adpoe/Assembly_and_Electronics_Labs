...
_subString:   
		...
		sw   $t4, 20($sp)     # store $$t4 in position 20 of stack frame
		
		# Operations to check edge cases
		jal _strLength
		addi $t4, $v0, 0                # store string length in $t4
		blt  $a2, $zero, startNeg       # branch if start index < 0
		blt  $a3, $zero, endNeg         # branch if end index < 0
		bgt  $a3, $t4, endTooBig        # branch if end index > strLength
		bgt  $a2, $a3, startTooBig      # branch if start index > end index
		j operation                     # else, go to operation segment
		
  startNeg:      addi $a2, $zero, 0             # if start index is negative, set it to 0
  	         j operation  
  
  endNeg:        addi $a3, $zero, 0             # if end index is negative, set it to 0
   	         j operation
   	       
  endTooBig:     addi $a3, $v0, 0               # if end index > strLength, set it to strlength	
                 j operation
               
  startTooBig:   addi $a2, $a3, 0	        # if start index > end index, set it to value of end index
  ...
  operation:	 add   $t0, $zero, $a1      # Set $t0 to the value of $a1 (output buffer address)
    ...
    exitSub:    add  $v0, $zero, $a1      # store the START address of the OUTPUT BUFFER into $v0
 		lw   $t4, 20($sp)         # restore value of $t4
