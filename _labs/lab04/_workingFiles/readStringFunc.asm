...
manipulateData: 
	...
	la $a0, buffer         # Confirm:  Address of null terminated string is in $a0
...
##############################################################################	
	...
	la  $a0, buffer        # Confirm:   Address of BUFFER -original string with newline removed
			       # is the input string 
...
## maybe just replace everything with zero... which is NOT.... in the substring
...
_subString:   
  ...
  operation:	 add   $a1, $a1, $zero      # Assert:  $a1 stores the address of the buffer 
           	 add   $t0, $zero, $a1      # Set $t0 to the value of $a1 (output buffer address)
    ...
    subLoop:     beq   $t1, $t2, exitSub
    		 sb    $t1, 0($t0)          # Store the first byte of the START address INTO the 0th byte of BUFFER
