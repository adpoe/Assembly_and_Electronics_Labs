...
remove:
        lb $a3,inputStr($s0)          # Load character at index
        ...
        sb $0, inputStr($s0)          # Add the terminating character in its place
skip:		
############################################################################
#############  Needs cleaned ###############################################
############################################################################
...
printResult:			     # If match was found, Lookup corresponding age in AGES 
	la  $a1, ages                # $a1 = ages
	add $t2, $a1, $t0            # $t2 = the byte address in $a1 + number of iterations we performed 
				     # in the main loop
	lb $t2, 0($t2)	 	     # load ONLY the byte we are addressing into $t2
	...
	addi $a0, $t2, 0             # $a0 = the byte we've indexed at $t3, the age we are looking for
